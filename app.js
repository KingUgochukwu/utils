...
const uploadsClient = mbxUploads({accessToken: 'sk.eyJ1IjoiZmlyZXRyYWMyMDIwIiwiYSI6ImNrenNwemR2eDBnMHAybnAzeHNnZmFzbDkifQ.s3PMvStAQW4pSxckEKoLVQ'}); 
const tilesetClient = mbxTilesets({accessToken: 'sk.eyJ1IjoiZmlyZXRyYWMyMDIwIiwiYSI6ImNrenNwemR2eDBnMHAybnAzeHNnZmFzbDkifQ.s3PMvStAQW4pSxckEKoLVQ'});
const stylesClient = mbxStyles({accessToken: 'sk.eyJ1IjoiZmlyZXRyYWMyMDIwIiwiYSI6ImNrenNwemR2eDBnMHAybnAzeHNnZmFzbDkifQ.s3PMvStAQW4pSxckEKoLVQ'});
...
async function createUpload(filePath){
    ...
    var result = await uploadsClient.createUpload({
...
async function downloadWeatherWarnings(){
    const filePath = 'weather_warning2.json'
...
const main = async ()=>{
    ...
    try {
        ...
        getCredentials().then(creds => credentials = creds)
        .then(() => putFileOnS3(credentials))
        .then(() => uploadsClient.createUpload({
...
const manipulateTileset = async ()=>{
    ...
    try{
       var result = await tilesetClient.listTilesetSources()
       .send()
...
async function createTileSets(){
    try{
        const tilesetClient = mbxTilesets({accessToken: 'sk.eyJ1IjoiZmlyZXRyYWMyMDIwIiwiYSI6ImNrenNwemR2eDBnMHAybnAzeHNnZmFzbDkifQ.s3PMvStAQW4pSxckEKoLVQ'})
        var watch = await tilesetClient.createTilesetSource({
            id: 'updatedSource', 
...
async function updateTileset(){
    try {  
        var res = await tilesetClient.updateRecipe({
            tilesetId: 'firetrac2020.weather_warnings',
            recipe: {
              version: 1,
              layers: {
...
async function test(){
   ...
   async function  publishTileset(){
     var watch = await tilesetClient.publishTileset({
        tilesetId: 'firetrac2020.weather_warnings'
      })
      .send()
      var result = ''
    ...
    async function updateSyle(){
      var styles = await stylesClient.updateStyle({
        styleId: 'ckx4zc7h50ctl15n9qnxefjaj', 
    ...
    async function createStyle(){
      var styles = await stylesClient.createStyle({
        style: require('./WW_style.json')
      }).send(); 
      var test = ""
    }
    ...
    function formatForStyles(){
        ...
        for (let i = 0; i < arrayKey.length; i++) {
          ...
          const hsl = rgbToHsl(element.symbol.color[0],element.symbol.color[1], element.symbol.color[2]); 
          result.push([type]); 
          result.push(hsl)
        }
        ...
        writeToFile.write(JSON.stringify(
          {paint : result }
        )); 
    ...
    function rgbToHsl(a, r, e) {
      ...
      return `hsl(${Math.round(360 * t)}, ${Math.round(100 * h)}%, ${Math.round(100 * c)}%)`
    }
